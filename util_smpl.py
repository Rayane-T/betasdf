...
def batch_axisangle_quat(theta):
    #theta N x 3 24*3
    batch_size = theta.shape[0]
...
def batch_global_rigid_transformation(Rs, Js, parent, rotate_base = False):
    ...
    #turn to homo
    def make_A(R, t): #R,t  n*3*3  n*3*1
        R_homo = F.pad(R, [0, 0, 0, 1, 0, 0]) #R is n*4*3
        t_homo = torch.cat([t, Variable(torch.ones(N, 1, 1)).to(R.device)], dim = 1) #n*4*1
        return torch.cat([R_homo, t_homo], 2) #n*4*4
    ...
    for i in range(1, parent.shape[0]):
        ...
        j_here = Js[:, i] - Js[:, parent[i]]  #local coords
        A_here = make_A(Rs[:, i], j_here)  #n*4*4
        
        res_here = torch.matmul(results[parent[i]], A_here) 
        results.append(res_here) #n*4*4

    results = torch.stack(results, dim = 1) #n*24*4*4
    
    new_J = results[:, :, :3, 3] #n*24*3  --after rotation

    Js_w0 = torch.cat([Js, Variable(torch.zeros(N, 24, 1, 1)).to(Rs.device)], dim = 2) #n*24*4*1 #0 is vector
...
if __name__ == '__main__':
    aa=torch.randn(1,24,3)
    res=batch_axisangle_quat(aa)
    print(res.size())
